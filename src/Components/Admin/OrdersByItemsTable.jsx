...
import { useGetOrdersByItemsQuery } from "../../Store/Api/admin/orders";
...
const OrdersByItemsTable = ({ params, setParams }) => {
  const { data, isLoading } = useGetOrdersByItemsQuery(params);
  const ProductsRows = data?.orderItems?.map((order) =>
    productsCreateData(
      order.orderId,
      order.user.name,
...
export default OrdersByItemsTable;
